...
ADXL345::ADXL345() {
    ...
    dev_address = ADXL345_ADDR_ALT_LOW;
...
ADXL345::init(int16_t address) {
    ...
    if(buf != ADXL345_IDENTITY) {
        Serial.print("ADXL345_IDENTITY_ERROR: Should be ");
        Serial.print(ADXL345_IDENTITY);
        Serial.print(" Read ");
        Serial.println(buf);
        return false;
    }
...
void
ADXL345::readFrom(byte address, int16_t num, byte buff[]) {
    Wire.beginTransmission(dev_address); // start transmission to device
    ...
    if(i != num) {
        ...
        Serial.print("ADXL345_READ_ERROR: Requested ");
        Serial.print(num);
