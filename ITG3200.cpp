...
ITG3200::ITG3200() {
    dev_address = ITG3200_ADDR_AD0_LOW;
...
bool
ITG3200::init(uint16_t  _address) {
...
ITG3200::zeroCalibrate(uint16_t totSamples) {
    ...
    tmpOffsets[0] = tmpOffsets[0] / (float)totSamples;
    tmpOffsets[1] = tmpOffsets[1] / (float)totSamples;
    tmpOffsets[2] = tmpOffsets[2] / (float)totSamples;
    setOffsets(-int16_t(tmpOffsets[0]+0.5),
               -int16_t(tmpOffsets[1]+0.5),
               -int16_t(tmpOffsets[2]+0.5));
}
...
ITG3200::reset() {
    ...
    delay(4*GYROSTART_UP_DELAY); //gyro startup
}
...
ITG3200::readmem(uint8_t address, uint8_t length, uint8_t buffer[]) {
    ...
    Wire.requestFrom(dev_address, length);// send data n-bytes read
    uint8_t i = 0; 
